...
impl AdaptiveAudioNormalizer {
    ...
    pub fn normalize(
        ...
        self.bass_peak = (self.bass_peak * self.peak_decay).max(bass).max(self.bass_avg * 1.5);
        self.mid_peak = (self.mid_peak * self.peak_decay).max(mid).max(self.mid_avg * 1.5);
        self.high_peak = (self.high_peak * self.peak_decay).max(high).max(self.high_avg * 1.5);
...
impl AudioState {
    ...
    pub fn update_smoothing(&mut self, dt: f32, smoothing_rate: f32, beat_attack: f32, beat_decay: f32) {
    ...
    pub fn update_fake(&mut self, _dt: f32) {
        ...
        if self.beat < 0.01 { self.beat = 0.0; }
        if self.bass < 0.01 { self.bass = 0.0; }
        if self.mid < 0.01 { self.mid = 0.0; }
        if self.high < 0.01 { self.high = 0.0; }
        if self.amplitude < 0.01 { self.amplitude = 0.0; }
...
pub fn analyze_audio(path: &str, fps: u32) -> anyhow::Result<AudioAnalysis> {
    ...
    for frame_idx in 0..total_frames {
        ...
        let sub_bass = spectrum[1..sub_bass_bin.max(2)].iter().sum::<f32>() 
            / (sub_bass_bin - 1).max(1) as f32;
        let bass = spectrum[sub_bass_bin..bass_bin.max(sub_bass_bin + 1)].iter().sum::<f32>() 
            / (bass_bin - sub_bass_bin).max(1) as f32;
        let mid = spectrum[bass_bin..mid_bin.max(bass_bin + 1)].iter().sum::<f32>() 
            / (mid_bin - bass_bin).max(1) as f32;
        let high = spectrum[mid_bin..high_bin.max(mid_bin + 1)].iter().sum::<f32>() 
            / (high_bin - mid_bin).max(1) as f32;
        
        // Spectral flux for beat detection
        let spectral_flux: f32 = spectrum.iter()
        ...
        let beat = if spectral_flux > avg_energy * 1.3 { 1.0 } else { 0.0 };
