...
struct ParticleStudioApp {
    ...
    accumulated_pause_duration: std::time::Duration,
    ...
    fn load_audio(&mut self, path: String) {
        ...
        match self.audio_sys.load_file(path, fps) {
            Ok(()) => {
                ...
                // Initialize sync timer for new track
                self.playback_start_time = Some(Instant::now());
                self.pause_start_time = None;
                self.accumulated_pause_duration = std::time::Duration::ZERO;
...
impl eframe::App for ParticleStudioApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if let Some(ref rx) = self.gpu_export_progress_rx {
            while let Ok(msg) = rx.try_recv() {
                match msg {
                    gpu_export::GpuExportMessage::Progress {
                        current,
                        total,
                        fps,
                    } => {
        ...
        if let Some(start_time) = self.playback_start_time {
            if self.is_playing {
                // Calculate actual elapsed time of track
                let elapsed = start_time
                    .elapsed()
                    .saturating_sub(self.accumulated_pause_duration)
                    .as_secs_f32();
                ...
                if let Some(ref analysis) = self.audio_sys.analysis {
                    ...
                    if self.current_frame < analysis.total_frames {
                        ...
                        self.audio_state
                            .update_from_frame(&frame, self.config.audio.smoothing);
                    } else {
        ...
        if self.audio_sys.analysis.is_none() {
            // Fake audio for demo
        ...
        // Update visualizers
        self.spectrum
            .update(&self.audio_state, &self.config.spectrum);
        self.waveform
            .update(&self.audio_state, &self.config.waveform);
...
impl ParticleStudioApp {
    fn render_top_bar(&mut self, ctx: &egui::Context) {
        egui::TopBottomPanel::top("top_bar").show(ctx, |ui| {
            ui.horizontal(|ui| {
                    ...
                    if self.is_playing {
                        ...
                        if ui.button("▶ Play").clicked() {
                            ...
                            if self.playback_start_time.is_none() {
                                ...
                                self.playback_start_time = Some(Instant::now());
                                self.accumulated_pause_duration = std::time::Duration::ZERO;
                            } else if let Some(pause_time) = self.pause_start_time {
                                // Resume from pause
                ...
                if ui
                    .selectable_label(self.config.particles.adaptive_audio_enabled, adaptive_text)
                    .on_hover_text("Automatically adjust to track dynamics")
                    .clicked()
                {
                    self.config.particles.adaptive_audio_enabled =
                        !self.config.particles.adaptive_audio_enabled;
                    if self.config.particles.adaptive_audio_enabled {
    ...
    fn render_particles_settings(&mut self, ui: &mut egui::Ui) {
        ...
        ui.label("Gravity");
        ui.add(egui::Slider::new(
            &mut self.config.particles.gravity,
            -2.0..=2.0,
        ));

        ui.label("Size Variation");
        ui.add(egui::Slider::new(
            &mut self.config.particles.size_variation,
            0.0..=1.0,
        ));

        ui.label("Glow Intensity");
        ui.add(egui::Slider::new(
            &mut self.config.particles.glow_intensity,
            0.0..=1.0,
        ));

        ui.label("Damping");
        ui.add(egui::Slider::new(
            &mut self.config.particles.damping,
            0.0..=10.0,
        ));
        ...
        ui.checkbox(
            &mut self.config.particles.audio_reactive_spawn,
            "Audio-Reactive Spawn",
        );
        ...
        if self.config.particles.audio_reactive_spawn {
            ...
            ui.horizontal(|ui| {
                if ui
                    .checkbox(
                        &mut self.config.particles.adaptive_audio_enabled,
                        "🎵 Adaptive Audio",
                    )
                    .changed()
                {
            ...
            if self.config.particles.adaptive_audio_enabled {
                ...
                ui.add(
                    egui::Slider::new(&mut self.config.particles.adaptive_strength, 0.0..=2.0)
                        .suffix("")
                        .text(""),
                );
                ...
                ui.add(egui::Slider::new(
                    &mut self.config.particles.adaptive_window_secs,
                    1.0..=5.0,
                ));

                ui.label("Bass Sensitivity");
                ui.add(egui::Slider::new(
                    &mut self.config.particles.bass_sensitivity,
                    0.5..=2.0,
                ));

                ui.label("Mid Sensitivity");
                ui.add(egui::Slider::new(
                    &mut self.config.particles.mid_sensitivity,
                    0.5..=2.0,
                ));

                ui.label("High Sensitivity");
                ui.add(egui::Slider::new(
                    &mut self.config.particles.high_sensitivity,
                    0.3..=1.5,
                ));
            } else {
                ui.label("Spawn Threshold");
                ui.add(egui::Slider::new(
                    &mut self.config.particles.audio_spawn_threshold,
                    0.0..=0.3,
                ));
            }

            ui.checkbox(
                &mut self.config.particles.fade_without_audio,
                "Fade Without Audio",
            );
            ...
            ui.add(egui::Slider::new(
                &mut self.config.particles.beat_burst_strength,
                0.0..=3.0,
            ));

            ui.label("Fade Attack (appear speed)");
            ui.add(egui::Slider::new(
                &mut self.config.particles.fade_attack_speed,
                1.0..=10.0,
            ));

            ui.label("Fade Release (fade speed)");
            ui.add(egui::Slider::new(
                &mut self.config.particles.fade_release_speed,
                0.5..=5.0,
            ));

            ui.add_space(8.0);
            ui.checkbox(
                &mut self.config.particles.spawn_from_center,
                "Spawn from Center",
            );
            if self.config.particles.spawn_from_center {
                ui.label("Spawn Radius");
                ui.add(egui::Slider::new(
                    &mut self.config.particles.spawn_radius,
                    10.0..=200.0,
                ));
            }
        }
        ...
        ui.checkbox(
            &mut self.config.particles.volumetric_rendering,
            "Volumetric (Smooth Gradients)",
        );
        if self.config.particles.volumetric_rendering {
            ui.label("Gradient Steps");
            ui.add(egui::Slider::new(
                &mut self.config.particles.volumetric_steps,
                8..=48,
            ));
        }
        ...
        ui.horizontal(|ui| {
            ui.selectable_value(
                &mut self.config.particles.shape,
        ...
        egui::ComboBox::from_label("")
            ...
            .show_ui(ui, |ui| {
                for (i, name) in self.color_scheme_names.iter().enumerate() {
                    if ui
                        .selectable_value(&mut self.config.color_scheme_index, i, name)
                        .changed()
                    {
                        self.particles
                            .update_palette(&self.config.get_color_scheme());
    ...
    fn render_audio_settings(&mut self, ui: &mut egui::Ui) {
        ...
        ui.add(egui::Slider::new(
            &mut self.config.audio.beat_sensitivity,
            0.1..=2.0,
        ));
        ...
        ui.label("Bass");
        ui.add(egui::Slider::new(
            &mut self.config.audio.bass_response,
            0.0..=3.0,
        ));

        ui.label("Mid");
        ui.add(egui::Slider::new(
            &mut self.config.audio.mid_response,
            0.0..=3.0,
        ));

        ui.label("High");
        ui.add(egui::Slider::new(
            &mut self.config.audio.high_response,
            0.0..=3.0,
        ));
        ...
        ui.add(egui::Slider::new(
            &mut self.config.audio.beat_explosion_strength,
            0.0..=10.0,
        ));

        ui.label("Beat Size Pulse");
        ui.add(egui::Slider::new(
            &mut self.config.audio.beat_size_pulse,
            0.0..=5.0,
        ));
    }
    ...
    fn render_effects_settings(&mut self, ui: &mut egui::Ui) {
        ui.collapsing("Bloom", |ui| {
            ...
            ui.label("Intensity");
            ui.add(egui::Slider::new(
                &mut self.config.visual.bloom_intensity,
                0.0..=2.0,
            ));
            ui.label("Radius");
            ui.add(egui::Slider::new(
                &mut self.config.visual.bloom_radius,
                5.0..=50.0,
            ));
            ui.label("Threshold");
            ui.add(egui::Slider::new(
                &mut self.config.visual.bloom_threshold,
                0.0..=1.0,
            ));
        });
        ...
        ui.collapsing("Motion & Vignette", |ui| {
            ...
            ui.add(egui::Slider::new(
                &mut self.config.visual.motion_blur,
                0.0..=0.9,
            ));
            ui.label("Vignette");
            ui.add(egui::Slider::new(
                &mut self.config.visual.vignette_strength,
                0.0..=1.0,
            ));
            ui.label("Film Grain");
            ui.add(egui::Slider::new(
                &mut self.config.visual.film_grain,
                0.0..=0.2,
            ));
        });
        ...
        ui.collapsing("MilkDrop Effects", |ui| {
            ...
            if self.config.visual.echo_enabled {
                ui.label("Zoom");
                ui.add(egui::Slider::new(
                    &mut self.config.visual.echo_zoom,
                    1.0..=1.1,
                ));
                ui.label("Rotation");
                ui.add(egui::Slider::new(
                    &mut self.config.visual.echo_rotation,
                    -0.1..=0.1,
                ));
                ui.label("Alpha");
                ui.add(egui::Slider::new(
                    &mut self.config.visual.echo_alpha,
                    0.0..=1.0,
                ));
            }
            ...
            if self.config.visual.radial_blur_enabled {
                ui.add(egui::Slider::new(
                    &mut self.config.visual.radial_blur_amount,
                    0.0..=0.5,
                ));
            }
        });
        ...
        ui.collapsing("Tone Mapping", |ui| {
            ui.label("Exposure");
            ui.add(egui::Slider::new(
                &mut self.config.visual.exposure,
                0.5..=2.0,
            ));
            ui.label("Contrast");
            ui.add(egui::Slider::new(
                &mut self.config.visual.contrast,
                0.5..=2.0,
            ));
            ui.label("Saturation");
            ui.add(egui::Slider::new(
                &mut self.config.visual.saturation,
                0.0..=2.0,
            ));
            ui.label("Gamma");
        ...
        ui.collapsing("UI Controls", |ui| {
            ui.checkbox(
                &mut self.config.visual.show_audio_meters,
                "Show Audio Meters",
            );
    ...
    fn render_spectrum_settings(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            ui.selectable_value(
                &mut self.config.spectrum.style,
                SpectrumStyle::MirrorBars,
                "Mirror",
            );
        ...
        ui.add(egui::Slider::new(
            &mut self.config.spectrum.bar_height_scale,
            0.1..=2.0,
        ));

        ui.label("Smoothing");
        ui.add(egui::Slider::new(
            &mut self.config.spectrum.smoothing,
            0.0..=0.99,
        ));

        ui.label("Position Y");
        ui.add(egui::Slider::new(
            &mut self.config.spectrum.position_y,
            0.0..=1.0,
        ));
    }
    ...
    fn render_waveform_settings(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ui.selectable_value(
                &mut self.config.waveform.style,
        ...
        ui.label("Thickness");
        ui.add(egui::Slider::new(
            &mut self.config.waveform.thickness,
            1.0..=10.0,
        ));

        ui.label("Amplitude");
        ui.add(egui::Slider::new(
            &mut self.config.waveform.amplitude,
            50.0..=400.0,
        ));

        ui.label("Position Y");
        ui.add(egui::Slider::new(
            &mut self.config.waveform.position_y,
            0.0..=1.0,
        ));
    ...
    fn render_export_settings(&mut self, ui: &mut egui::Ui) {
        ...
        if self.gpu_export_available {
            ...
            ui.checkbox(
                &mut self.use_gpu_export,
                "Use GPU-accelerated export (3-5x faster)",
            );
        } else {
        ...
        if !self.export_is_exporting {
                    ...
                    if let Some(ref path) = &self.export_output_path {
                        if let Some(ref analysis) = &self.audio_sys.analysis {
                            ...
                            if self.use_gpu_export && self.gpu_export_available {
                                ...
                                println!(
                                    "Resolution: {}x{}",
                                    self.config.export.width, self.config.export.height
                                );
                                ...
                                println!(
                                    "GPU export started with {} encoder!",
                                    self.detected_encoder.name()
                                );
                            } else {
            ...
            let mode_str = if self.gpu_export_progress_rx.is_some() {
                "GPU"
            } else {
                "CPU"
            };
            let frame_info = format!(
                "Frame {}/{} ({} Export)",
                self.export_current_frame, self.export_total_frames, mode_str
            );
            ui.label(frame_info);
            ...
            if self.gpu_export_progress_rx.is_some() && self.gpu_export_fps > 0.0 {
                ...
                ui.label(format!(
                    "🚀 {:.1} fps ({:.1}x realtime)",
                    self.gpu_export_fps, speed_ratio
                ));
            } else {
    ...
    fn draw_audio_meters(&self, ui: &egui::Ui, rect: egui::Rect, painter: &egui::Painter) {
        ...
        painter.rect_filled(
            bass_rect,
            2.0,
            egui::Color32::from_rgba_unmultiplied(50, 50, 80, 100),
        );
        ...
        let mid_rect = egui::Rect::from_min_size(
            egui::Pos2::new(
                rect.left() + margin + meter_width + 4.0,
                rect.bottom() - margin - meter_height,
            ),
            egui::Vec2::new(meter_width, meter_height),
        );
        painter.rect_filled(
        ...
        let high_rect = egui::Rect::from_min_size(
            egui::Pos2::new(
                rect.left() + margin + (meter_width + 4.0) * 2.0,
                rect.bottom() - margin - meter_height,
            ),
            egui::Vec2::new(meter_width, meter_height),
        );
