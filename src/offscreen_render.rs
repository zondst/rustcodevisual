...
use egui::Color32;
use crate::particles::{Particle, ParticleEngine};
use crate::config::{AppConfig, ParticleShape, BlendMode};
...
impl FrameRenderer {
    ...
    fn draw_volumetric_glow(&mut self, cx: f32, cy: f32, r: f32, color: Color32, alpha: f32, blend: &BlendMode) {
        if r <= 1.0 || alpha < 0.01 {
            return;
        }
                ...
                if dist_sq <= r_sq {
                    ...
                    if a > 0.001 {
                        ...
                        match blend {
                            ...
                            _ => {
                                self.hdr_buffer[idx] = self.hdr_buffer[idx] * (1.0 - a) + color.r() as f32 * a;
                                self.hdr_buffer[idx+1] = self.hdr_buffer[idx+1] * (1.0 - a) + color.g() as f32 * a;
                                self.hdr_buffer[idx+2] = self.hdr_buffer[idx+2] * (1.0 - a) + color.b() as f32 * a;
                            }
                        }
                    }
                }
    ...
    fn draw_glow_particle(&mut self, cx: f32, cy: f32, r: f32, color: Color32, alpha: f32, blend: &BlendMode) {
        if r <= 0.5 || alpha < 0.01 {
            return;
        }
