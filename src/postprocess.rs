...
impl PostProcessor {
    ...
    pub fn process(
        ...
        if config.kaleidoscope_enabled {
            self.apply_kaleidoscope(&mut float_buffer, width, height, config.kaleidoscope_segments);
        }
        ...
        if config.chromatic_aberration > 0.0 {
            self.apply_chromatic_aberration(&mut float_buffer, width, height, config.chromatic_aberration);
        }
        ...
        if config.bloom_enabled && config.bloom_intensity > 0.0 {
            ...
            self.apply_bloom(&mut float_buffer, width, height, intensity, config.bloom_threshold, config.bloom_radius);
        }
        ...
        for level in (0..mip_levels - 1).rev() {
            ...
            for dy in 0..dst_h {
                for dx in 0..dst_w {
                    ...
                    if dst_idx < dst.len() {
                        dst[dst_idx][0] = dst[dst_idx][0] * (1.0 - combine_constant) + upsampled[0] * combine_constant;
                        dst[dst_idx][1] = dst[dst_idx][1] * (1.0 - combine_constant) + upsampled[1] * combine_constant;
                        dst[dst_idx][2] = dst[dst_idx][2] * (1.0 - combine_constant) + upsampled[2] * combine_constant;
                    }
                }
            }
        }
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                if rx >= 0.0 && rx < width as f32 && ry >= 0.0 && ry < height as f32 {
                    ...
                    if src_idx < echo.len() {
                        result[dst_idx][0] = buffer[dst_idx][0] * (1.0 - alpha) + echo[src_idx][0] * alpha;
                        result[dst_idx][1] = buffer[dst_idx][1] * (1.0 - alpha) + echo[src_idx][1] * alpha;
                        result[dst_idx][2] = buffer[dst_idx][2] * (1.0 - alpha) + echo[src_idx][2] * alpha;
                    }
                }
            }
        }
...
impl BackgroundRenderer {
    ...
    pub fn render(&self, buffer: &mut FrameBuffer, colors: &crate::config::ColorScheme, animated: bool, intensity: f32, audio: &AudioState) {
