...
impl Compositor {
    ...
    pub fn apply_postprocess(&mut self, config: &AppConfig, audio: &AudioState) {
        self.postprocess.process(&mut self.buffer, &config.visual, audio);
    }
    ...
    pub fn start(&mut self, config: &crate::config::ExportConfig) -> anyhow::Result<()> {
        ...
        if let Some(ref audio_path) = self.audio_path {
            args.extend(vec![
                ...
                "-c:a".to_string(), config.codec.clone(),
        ...
        args.extend(vec![
            "-c:v".to_string(), config.codec.clone(),
...
pub fn render_to_video(
    ...
    for frame_idx in 0..total_frames {
        ...
        particles.update(&config.particles, &audio_state, dt, None);
            ...
            for dx in -size..=size {
                if dx * dx + dy * dy <= size * size {
                    ...
                    if px >= 0 && px < width as i32 && py >= 0 && py < height as i32 {
                        ...
                        // Additive blend
                        pixel[0] = (pixel[0] as f32 + p.color.r() as f32 * edge_alpha).min(255.0) as u8;
                        pixel[1] = (pixel[1] as f32 + p.color.g() as f32 * edge_alpha).min(255.0) as u8;
                        pixel[2] = (pixel[2] as f32 + p.color.b() as f32 * edge_alpha).min(255.0) as u8;
                    }
                }
            }
        }
